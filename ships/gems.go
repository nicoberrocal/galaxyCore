...
func initAffinity() {
	...
	set := func(a, b GemFamily, aff Affinity) {
		if AffinityGraph[a] == nil {
			AffinityGraph[a] = map[GemFamily]Affinity{}
		}
		if AffinityGraph[b] == nil {
			AffinityGraph[b] = map[GemFamily]Affinity{}
		}
...
func getAffinity(a, b GemFamily) Affinity {
	if a == b {
		return Affinity{Weight: 1, BaseSuccess: 1.0, Efficiency: 0.90, OriginHint: defaultOrigin(a, 1)}
	}
...
func SynthesizeGems(inputs []Gem) GemMergeResult {
	...
	if len(inputs) == 3 && isPureTriple(inputs) {
		...
		if !ok {
			res.FailureReason = "cannot upgrade"
			return res
		}
	...
	if len(inputs) == 3 && isRelicSingularityCoreInputs(inputs) {
		...
		if rng.Float64() <= p {
			...
			for _, in := range inputs {
				mods = CombineMods(mods, scaleMods(in.Mods, 0.40))
			}
			...
			for _, in := range inputs {
				if in.Tier > 1 {
					in.Tier = maxInt(1, in.Tier-1)
					in.ID = GemID(familyID(in.Family, in.Tier))
				}
				back = append(back, in)
			}
			res.Returned = back
	...
	if len(fams) < 2 {
		res.FailureReason = "not enough distinct families for mixed merge"
		return res
	}
	if len(fams) > 2 {
		res.FailureReason = "3-family hybrids not supported (except relic)"
		return res
	}
	...
	if rng.Float64() <= p {
		...
		if back[idx].Tier > 1 {
			back[idx].Tier--
			back[idx].ID = GemID(familyID(back[idx].Family, back[idx].Tier))
		}
		res.Returned = back
...
func isRelicSingularityCoreInputs(in []Gem) bool {
	...
	for _, g := range in {
		if g.Kind != KindHybrid || g.Tier < MaxGemTier {
			return false
		}
		names[g.Name] = true
	}
...
func tierFromEnergy(e float64, origin GemOrigin) int {
	best := 1
	for t := 1; t <= MaxGemTier; t++ {
		if energyForTierOrigin(t, origin) <= e {
			best = t
		}
	}
	return best
...
func hybridMods(fams []GemFamily, tier int, aff Affinity) StatMods {
	...
	base := ZeroMods()
	for _, f := range fams {
		base = CombineMods(base, gemTierMods(f, tier))
	}
	scale := 0.50
	if aff.Weight >= 0.4 {
		scale = 0.60
	} else if aff.Weight <= -0.3 {
		scale = 0.40
	}
